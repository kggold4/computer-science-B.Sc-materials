...
public class BottleWeightFW {
	...
	//**********************************************************	
	// the weight's matrix initialization
	...
	//**********************************************************
	public static void checkBottleWeight(int m, int n, int k){
		...
		printPath(paths, mat);
		System.out.println("--------The Shortest path from (0,0) -----------------");
		System.out.println(getPath(mat,n,m,k,paths));
		IsExistPath(0,0,4,0,mat,paths,n,m);
	}
	
	/**
	 * Returns the string represents the path from (0,0) to k
	 */
	public static String getPath(int[][] mat,int n, int m, int k, String [][]paths) {
		System.out.println("Is Exist path from (0,0) to "+k+"?");
		if(mat[index(0,0,n)][index(k>n?k:0,k>n?0:k,n)] == infinity) {
			return "no path!";
		}
		return paths[index(0,0,n)][index(k>n?k:0,k>n?0:k,n)];
	}
	//**********************************************************
	/**
	 * Print the path from (i1,j1) to (i2,j2)
	 */
	static void IsExistPath(int i1, int j1, int i2, int j2, int[][] mat, String[][] paths, int n, int m) {
		i1 = i1>j1?i1:j1;
		j1 = i1>j1?j1:i1;
		i2 = i2>j2?i2:j2;
		j2 = i2>j2?j2:i2;
	...
	public static void main(String[] args) {
		int m=5, n=3, k=4;//m-the first bottle, n-the second bottle
		System.out.println("m = "+ m + ",  n = "+n+"\n");
		checkBottleWeight(m, n, k);
	}
}
