...
public class HuffmanAlgorithmPriorityQueue {//O(nlog(n))
	private Node nodes[];
	...
	// constructor
	public HuffmanAlgorithmPriorityQueue(int freq[]){//O(n)+O(n)+O(nlog(n))
		...
		for (int i = 0; i < numOfLeaves; i++) {//O(nlog(n))
			queue.add(nodes[i]); 
		}
		for (int i = 0; i < numOfLeaves; i++) {//O(n)
			codes[i] = new String();
	...
	public static void main(String[] args) {
		...
		HuffmanAlgorithmPriorityQueue hh =new HuffmanAlgorithmPriorityQueue(freq1);
		hh.HuffmanAlgorithm();
		hh.printCode();
		System.out.println();
		HuffmanAlgorithmPriorityQueue hh1 =new HuffmanAlgorithmPriorityQueue(freq2);
		hh1.HuffmanAlgorithm();
		hh1.printCode();		
	}
