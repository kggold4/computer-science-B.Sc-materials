...
import practice_trees.PrintTrees;
...
// binary search tree
    ...
    public Node root;
        ...
        // add method
        public void add(int data) { this.root = add(data, this.root, this.root); }
        private Node add(int data, Node node, Node prev) {
            if(node == null) {
                Node newNode = new Node(data);
                newNode.parent = prev;
                return newNode;
            }
            else if(data < node.data) node.left = add(data, node.left, node);
            else node.right = add(data, node.right, node);
            return node;
        }
