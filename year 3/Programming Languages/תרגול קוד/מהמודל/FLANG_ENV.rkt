...
(define (extend id expr env)
  (cons (list id expr) env))
|#

(: lookup : Symbol ENV -> VAL)
(define (lookup name env)
  (cases env
    [(EmptyEnv) (error 'lookup "free instance ~s" name)]
    [(Extend id val rest-env) (if (eq? name id) val (lookup name rest-env))]
   ))
...
1. eval(N, env) = N
2. eval({+ E1 E2}, env) = if eval(E1, env), eval(E2, env) return numbers
                                 eval(E1, env) + eval(E2, env)
                            otherwise: error!
3. eval({- E1 E2}, env) = if eval(E1, env), eval(E2, env) return numbers
                                 eval(E1, env) - eval(E2, env)
                            otherwise: error!
4. eval({* E1 E2}, env) = if eval(E1, env), eval(E2, env) return numbers
                                 eval(E1, env) * eval(E2, env)
                            otherwise: error!
5. eval({/ E1 E2}, env) = if eval(E1, env), eval(E2, env) return numbers
                                 eval(E1, env) / eval(E2, env)
                            otherwise: error!
6. eval(id, env) = lookup(id,env)

7. eval({with {x E1} E2}, env) = eval(E2, extend(x,eval(E1,env),env))

8. eval({fun {x} E}, env) = <{fun {x} E}, env> ; closure


9. eval({call E1 E2}, env)= if <{fun {x} E},f-env> <--eval(E1, env)
                            eval(E, extend(E2, env), f-env)
                         otherwise: error!
...
(define (NumV->Number arg)
  (cases arg
    [(NumV n) n]
    [else (error 'NumV->Number "expects a number, got: ~s" arg)]
   ))
(NumV (op (NumV->Number arg1) (NumV->Number arg2))))
...
(define (eval exp env)
  (cases exp
    ...
    [(With name named-expr body)
     (eval body (Extend name (eval named-expr env)
                        env))]
    [(Id name) (lookup name env)]
    [(Fun name body) (FunV name body env)] ;-- clousure - a function which is a closed object
    [(Call fun-expr arg-expr)
     (let ([fval (eval fun-expr env)])
       (cases fval
         [(FunV name body f-env) (eval body (Extend name
...
(: run : String -> Number)
(define (run code)
  (let ([res (eval (parse code) (EmptyEnv))])
    (cases res
      [(NumV n) n]
...
#|
{with {x 3} ;[] 
      {with {f {fun {y} {+ x y}}};[(x 3)]
            {with {x 5}; [(f {fun {y} {+ x y}})(x 3)]
                  {call f x = 5}}}}; [(x 5)(f {fun {y} {+ x y}})(x 3)]
  

{+ x = 5 y = 5} [(y 5) (x 5)(f {fun {y} {+ x y}})(x 3)]
